...
- **8.4 — Reintento de Envío (D-F6)** — ✅ Completado (2026-09-02, v2.36.0), posterior al cierre del roadmap 8.1→8.3. Un fallo de `sendMessage` se descartaba en silencio (job quedaba `done`, cliente sin respuesta, sin reintento). Reintento acotado al envío (hasta 3 intentos con backoff corto, `jobs/inbound-message.job.js`) — no al job completo, para no arriesgar efectos duplicados de un reprocesamiento total. Sin cambios en `contexts/` ni en el motor conversacional. Ver `docs/history/ENTREGABLE_8_4_COMPLETION_REPORT.md`.
