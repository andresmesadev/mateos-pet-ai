...
- **6.6 — Operación Centralizada** — Fase B (saneamiento) ✅ Completada (2026-08-10, v2.21.0); Fase A (formalización) pendiente de autorización separada. La auditoría encontró que la "visibilidad de todos los establecimientos" que el roadmap describía **ya existía, de forma accidental**: el patrón `tenantId ? {...} : {}`, replicado en 13 repositorios de 6+ contextos (Finanzas, Staff, Automatizaciones, Empleados Digitales, Comunicación, Eventos), otorgaba una vista cross-tenant sin filtro a cualquier superadmin sin `tenantId`, sin control de acceso ni auditoría. Decisión adoptada: enfoque "B luego A" — cerrar primero el riesgo de seguridad, formalizar después la capacidad deliberada. Fase B cerrada en un único choke point compartido por todos los contextos (`resolveTenant.js`, aplicado globalmente a `/api/dashboard`), sin modificar ninguno de los 13 repositorios: un superadmin sin `tenantId` ahora debe declarar la intención explícitamente (`X-View-All-Tenants`) o es rechazado con 403. Sin cambios de schema, ni Reconciliación Arquitectónica, ni contextos de negocio tocados. Ver `docs/history/ENTREGABLE_6_6_COMPLETION_REPORT.md` y `docs/history/ENTREGABLE_6_6_GATE_REVIEW.md`.
