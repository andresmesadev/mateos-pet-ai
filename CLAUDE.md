...
- **No proponer funcionalidades de fases futuras.** La Fase 4 (Plataforma Comercial) está completa (4.1 → 4.4); la Fase 5 (Operaciones Inteligentes) está completa desde el punto de vista funcional (roadmap interno 5.1 → 5.4 cerrado oficialmente, v2.16.0) — el motor de Automatizaciones reacciona a cualquier Evento de Dominio certificado, no solo `CitaCompletada`, y la primera aplicación real de `AgentAutonomyLimit` ya existe en producción (Coordinador de Agenda IA). Cualquier trabajo posterior sobre infraestructura reactiva/automatizaciones/agentes corresponde a una nueva fase, a definir formalmente. Ver `docs/PLAN_MAESTRO.md` para el alcance vigente.
